...
> `TRITON_MSL_MEPT=0` escape hatch to the scalar/wrap-loop path. The flip gate
...
- **Default ON + `TRITON_MSL_MEPT=0` escape hatch.** Absent ⇒ register-array model
...
- `compiler.py` `_msl_cache_key`: the cache key must key on the **effective** flag
  (same default logic), so a default run and an explicit `TRITON_MSL_MEPT=1` run
  share a key, and `=0` gets a distinct key.
- Any other read site (the implementation plan greps `TRITON_MSL_MEPT` and flips
...
## Cache invalidation
Bump `CODEGEN_VERSION` in `triton_msl/__init__.py` (currently `"2026.06.09"`).
...
## Components (files)
- `triton_msl/codegen/generic_lowerer.py` — flip the `mept_enabled` default.
- `triton_msl/backend/compiler.py` — flip the cache-key flag default to match.
- `triton_msl/__init__.py` — bump `CODEGEN_VERSION`.
...
path it previously took only under `TRITON_MSL_MEPT=1`. A non-eligible kernel is
unaffected (eligibility still gates emission). `TRITON_MSL_MEPT=0` restores the
...
- **New default (no env var):** full upstream `test_core` = 5335/0; project suite
  ...
  require `TRITON_MSL_MEPT=1`; post-flip they pass with no env too — the new
  `test_mept_m5_default_gpu.py` asserts the no-env default explicitly.)
- **Escape hatch (`TRITON_MSL_MEPT=0`):** full upstream `test_core` = 5335/0;
