"""Base-path BLOCK-wide store under-coverage refusal.

A base-path tensor store emits PTR[k + lid] = val (one element per thread).
...
DROPS the rest — a silent-wrong (the tridec relay megakernel at block=256,
num_warps=4). The correct n>1 store paths are the MEPT register-array scatter,
the smem-backed path, and _loop_e-wrapped stores; when none apply the store
must refuse loudly. Launching with num_warps = BLOCK/32 (num_threads == BLOCK,
n=1) is verified correct.

A pure elementwise BLOCK-wide store is already covered correctly under the
default flag (it is MEPT register-array-scattered) or by the _loop_e wrap, so
it never reaches the base path. The base-path under-cover only happens when a
kernel is forced onto multipass (a 1-D full reduce → block_size = num_threads)
AND its multi-element value is NOT register-array / wrap covered (here a
rank-changing tl.reshape defeats the MEPT spine) AND a BLOCK-wide store sits in
...
_MODULE_KERNELS = (_undercover_store,) if HAS else ()
...
@requires_metal
def test_blockwide_store_refuses_when_undercover():
    """BLOCK=256 with num_warps=4 → 128 threads, n=2 per thread. The base path
    would store only one element per thread, silently dropping half of each
    ...
    with pytest.raises(MetalNonRecoverableError):
        _undercover_store[(1,)](X, OUT, OUT2, K, BLOCK=BLOCK, H=H, num_warps=4)


@requires_metal
def test_blockwide_store_correct_when_matched():
    ...
    OUT = torch.zeros(BLOCK, device="mps", dtype=torch.float32)
    OUT2 = torch.zeros(BLOCK, device="mps", dtype=torch.float32)
    _undercover_store[(1,)](X, OUT, OUT2, K, BLOCK=BLOCK, H=H, num_warps=8)
    # Last loop iteration (k = K-1) wins the OUT store.
    ref = X + X.sum() + (K - 1)
