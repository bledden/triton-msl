...
def test_msl_cache_key_changes_with_mept(monkeypatch):
    ...
    from triton_msl.backend.compiler import _msl_cache_key
    monkeypatch.delenv("TRITON_MSL_MEPT", raising=False)  # default ON
    default_on = _msl_cache_key("m", "o")
    monkeypatch.setenv("TRITON_MSL_MEPT", "0")            # escape hatch
    assert _msl_cache_key("m", "o") != default_on
    monkeypatch.setenv("TRITON_MSL_MEPT", "1")            # explicit ON == default
    assert _msl_cache_key("m", "o") == default_on
